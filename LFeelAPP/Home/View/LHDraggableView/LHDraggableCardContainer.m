...
@interface LHDraggableCardContainer ()
...
@implementation LHDraggableCardContainer
...
- (id)init {
    ...
    if (self) {
        ...
        _canDraggableDirection = LHDraggableDirectionLeft | LHDraggableDirectionLeft;
    }
    return self;
}
...
- (void)cardViewDirectionAnimation:(LHDraggableDirection)direction isAutomatic:(BOOL)isAutomatic undoHandler:(void (^)())undoHandler {
    //    NSLog(@"===================================%ld", direction);
    if (!_isInitialAnimation) { return; }
    UIView *view = [self getCurrentView];
    if (!view) { return; }
    
    __weak LHDraggableCardContainer *weakself = self;
    if (direction == LHDraggableDirectionDefault) {
    ...
    if (direction == LHDraggableDirectionRight || direction == LHDraggableDirectionLeft || direction == LHDraggableDirectionDown) {
        ...
        [UIView animateWithDuration:0.35
                         ...
                         animations:^{
                             ...
                             if (direction == LHDraggableDirectionLeft) {
                             ...
                             if (direction == LHDraggableDirectionRight) {
                             ...
                             if (direction == LHDraggableDirectionDown) {
    ...
    if (direction == LHDraggableDirectionUp) {
...
- (void)cardViewDefaultScale {
    if (self.delegate && [self.delegate respondsToSelector:@selector(cardContainderView:updatePositionWithDraggableView:draggableDirection:widthRatio:heightRatio:)]) {
        ...
        [self.delegate cardContainderView:self updatePositionWithDraggableView:[self getCurrentView]
                       draggableDirection:LHDraggableDirectionDefault
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(cardContainderView:updatePositionWithDraggableView:draggableDirection:widthRatio:heightRatio:)]) {
            if ([self getCurrentView]) {
                ...
                LHDraggableDirection direction = LHDraggableDirectionDefault;
                
                if (fabs(ratio_h) > fabs(ratio_w)) {
                    
                    if (ratio_h <= 0) {
                        // up
                        //                        NSLog(@"%ld /////// %ld", _canDraggableDirection, YSLDraggableDirectionDown);
                        
                        if (_canDraggableDirection & LHDraggableDirectionUp) {
                            direction = LHDraggableDirectionUp;
                        } else {
                            direction = ratio_w <= 0 ? LHDraggableDirectionLeft : LHDraggableDirectionRight;
                        }
                    } else {
                        // down
                        //                        NSLog(@"%ld ===== %ld", _canDraggableDirection, YSLDraggableDirectionDown);
                        
                        if (_canDraggableDirection & LHDraggableDirectionDown) {
                            direction = LHDraggableDirectionDown;
                        } else {
                            direction = LHDraggableDirectionDown;
                    ...
                    if (ratio_w <= 0) {
                        ...
                        if (_canDraggableDirection & LHDraggableDirectionLeft) {
                            direction = LHDraggableDirectionLeft;
                        } else {
                            direction = ratio_h <= 0 ? LHDraggableDirectionUp : LHDraggableDirectionDown;
                        }
                    } else {
                        // right
                        //                        NSLog(@"%ld +++++ %ld", _canDraggableDirection, YSLDraggableDirectionDown);
                        
                        if (_canDraggableDirection & LHDraggableDirectionRight) {
                            direction = LHDraggableDirectionRight;
                        } else {
                            direction = ratio_h <= 0 ? LHDraggableDirectionUp : LHDraggableDirectionDown;
                        }
                    }
    ...
    if (gesture.state == UIGestureRecognizerStateEnded || gesture.state == UIGestureRecognizerStateCancelled) {
        ...
        LHDraggableDirection direction = LHDraggableDirectionDefault;
        if (fabs(ratio_h) > fabs(ratio_w)) {
            if (ratio_h < - kDragCompleteCoefficient_height_default && (_canDraggableDirection & LHDraggableDirectionUp)) {
                // up
                direction = LHDraggableDirectionUp;
            }
            
            if (ratio_h > kDragCompleteCoefficient_height_default && (_canDraggableDirection & LHDraggableDirectionDown)) {
            ...
            if (ratio_w > kDragCompleteCoefficient_width_default && (_canDraggableDirection & LHDraggableDirectionRight)) {
                // right
                direction = LHDraggableDirectionRight;
            }
            
            if (ratio_w < - kDragCompleteCoefficient_width_default && (_canDraggableDirection & LHDraggableDirectionLeft)) {
                // left
                direction = LHDraggableDirectionLeft;
            }
        }
        ...
        if (direction == LHDraggableDirectionDefault) {
            [self cardViewDirectionAnimation:LHDraggableDirectionDefault isAutomatic:NO undoHandler:nil];
        } else {
