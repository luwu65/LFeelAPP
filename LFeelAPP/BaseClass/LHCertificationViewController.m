...
- (void)setHBK_NavigationBar {
    ...
    self.hbk_navgationBar = [HBK_NavigationBar HBK_setupNavigationBarWithTitle:@"实名认证" backAction:^{
        ...
        kVerifyText(self.photoBase64.length, @"请选择照片或拍照");
...
- (void)requestCertificationData {
    ...
    [LHNetworkManager PostWithUrl:kCertificationUrl parameter:@{@"real_name": self.realNameTF.text, @"id_num": self.IDCardTF.text, @"id_positive":self.photoBase64, @"user_id": kUser_id} success:^(id reponseObject) {
        NSLog(@"%@", reponseObject);
